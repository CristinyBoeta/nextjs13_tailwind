...
async function main() {
  const commitlinterrcFilePath = process.cwd() + '/commitlinterrc.json';

  try {
    const config = await readConfig(commitlinterrcFilePath);

    const commitMsgFilePath = config.gitPath
      ? `${config.gitPath}/COMMIT_EDITMSG`
      : '.git/COMMIT_EDITMSG';

    const commitMsgContent = await readFile(commitMsgFilePath);
...
async function lint(commitMsgContent, config, lang) {
  ...
  const { example, scope, invalidScope, subject, invalidSubject } =
    DESCRIPTIONS;
...
async function readConfig(filename) {
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      ...
      console.error(
        `${packageName}: ${RED}read commitlinterrc.json failed`,
        error
      );
    }
  }
  ...
  try {
    ...
    console.error(
      `${packageName}: ${RED}commitlinterrc.json ignored because of invalid json`
    );
  }
...
function displayError(
  ...
  const header = !showInvalidHeader
    ? ''
    : `\n  ${
        invalidFormat ? RED : YELLOW
      }************* ${invalidHeader} **************${EOS}`;

  const scopeDescription = scopeDescriptions.join('\n    ');
  const invalidScopeDescription = invalidScopeDescriptions.join('\n    ');
  const defaultInvalidScopeDescription = `scope can be ${emphasis(
    'optional'
  )}${RED}, but its parenthesis if exists cannot be empty.`;
  ...
  console.info(
    `${header}${
      invalid
        ? `
  ${label(`${commitMessage}:`)} ${RED}${message}${EOS}`
        : ''
    }${generateInvalidLengthTips(message, invalidLength, maxLen, minLen, lang)}
  ${label(
    `${correctFormat}:`
  )} ${GREEN}${decoratedType}${scope}: ${subject}${EOS}
  ...
  ${invalidSubject ? RED : YELLOW}subject:
    ${GRAY}${subjectDescription}${postSubjectDescription}${
      invalidSubject
        ? `${RED}
...
function didYouMean(message, { types, example }) {
  ...
  return message.replace(TYPE_REGEXP, (_, p1) =>
    p1 && p1 !== '()' ? `${suggestedType}${p1}:` : `${suggestedType}:`
  );
}
...
function describeTypes(mergedTypes, suggestedType = '') {
  const types = Object.keys(mergedTypes);
  const maxTypeLength = [...types].sort((t1, t2) => t2.length - t1.length)[0]
    .length;
...
function generateInvalidLengthTips(message, invalid, maxLen, minLen, lang) {
  if (invalid) {
    ...
    const tips = `${RED}${i18n.length} ${BOLD}${
      message.length
    }${EOS}${RED}. ${format(i18n.invalidLengthTip, max, min)}${EOS}`;
  ...
  return {
    'en-US': {
      ...
      i18n: {
        ...
        length: 'Length',
        invalidLengthTip:
          'Commit message cannot be longer than {1} characters or shorter than {2} characters',
      },
    },
    ...
    'zh-CN': {
      stereotypes: {
        ...
        docs: '更新文档或注释',
        style:
          '代码格式调整，对逻辑无影响：比如为按照 eslint 或团队风格修改代码格式。注意不是 UI 变更',
        refactor:
          '重构：不影响现有功能或添加功能。比如文件、变量重命名，代码抽象为函数，消除魔法数字等',
        test: '单测相关变更',
        ...
        ci: '持续集成相关变更',
        build:
          '代码构建相关变更：比如修复部署时的构建问题、构建脚本 webpack 或 gulp 相关变更',
      ...
      descriptions: {
        ...
        invalidScope: ['`scope` 可选，若有则必须加小括号'],
        subject: [
    ...
    'pt-BR': {
      ...
      i18n: {
        ...
        length: 'Comprimento',
        invalidLengthTip:
          'Mensagem de commit não pode ser maior que {1} caracteres ou menor que {2}',
      },
    },
    ...
    'es-ES': {
      stereotypes: {
        ...
        style:
          'Cambios que no afectan la ejecución del código (espacios en blanco, formato, falta de punto y coma, etc.).',
        refactor:
          'Un cambio de código que no afecta el funcionamiento existente (no corrige un error ni añade una función.)',
        test: 'Añadir pruebas que faltan o corregir las existentes.',
        chore:
          'Cambios en el proceso de construcción o en las herramientas y bibliotecas auxiliares, como la generación de documentación, sin afectar el código de producción.',
        // added
        perf: 'Un cambio de código que mejora el rendimiento.',
        ci: 'Cambios en sus archivos de configuración y scripts de CI.',
        build:
          'Cambios que afectan al sistema de construcción o a las dependencias externas (ejemplos de ámbitos: gulp, broccoli, npm).',
        temp: 'Cambio temporal que no se incluirá en su CHANGELOG.',
      },
      descriptions: {
        example:
          'docs: actualiza el README para añadir consejos para desarrolladores',
        scope: [
      ...
      i18n: {
        ...
        length: 'Longitud',
        invalidLengthTip:
          'El mensaje del commit no puede tener más de {1} caracteres, o menos de {2}',
      },
    },
  };
}
